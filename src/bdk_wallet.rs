...
impl Wallet {
    ...
    fn get_db_path(key_index: u32) -> String {
...
impl RegtestWallet for Wallet {
    ...
    /// Clear the database
    fn clear_db(key_index: u32) {
        let path = Wallet::get_db_path(key_index);
...
mod tests {
    ...
    fn test_bdk_wallet() -> Result<(), anyhow::Error> {
        ...
        let key_index = 13;
        Wallet::clear_db(key_index);

        let mut wallet = Wallet::new(config, key_index)?;
        ...
        let tx = psbt.extract_tx().expect("tx");
        // Broadcast the transaction
        let tx_hash = wallet.send_transaction(&tx)?;
