...
- `wallet/{identifier}/transfer-destinations/{funding_id}` — optional pending transfer destination metadata storing non-taproot `(destination_pubkey, amount_sat, vout)` records so controlled destination wallets can be credited on confirmation.
...
- `confirm_transfer` removes the pending transfer and original funding entry, then re-adds change under the same `funding_id` when change is non-zero. If non-taproot destination outputs pay to pubkeys belonging to locally controlled wallets, it also registers those outputs as funding entries named `received_{last_6_txid_hex}` (with numeric suffixes on collision).
