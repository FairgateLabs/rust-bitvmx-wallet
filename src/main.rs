    ...
    match &cli.command {
        Commands::SendToAddress {
            ...
            match wallet.send_to_address(to_address, *amount, *fee_rate) {
                ...
                Err(e) => eprintln!("Error sending to address {to_address} with amount {amount} satoshis: {e}"),
            }
        }
        ...
        Commands::Mine { num_blocks } => {
            let wallet = match Wallet::from_derive_keypair(config.bitcoin.clone(), config.wallet.clone(), 0, wallet_manager.key_manager.clone()) {
                Ok(w) => w,
                Err(e) => {
        ...
        Commands::SendToAddressAndMine {
            ...
            match wallet.fund_address(to_address, *amount) {
                ...
                Err(e) => eprintln!("Error funding address {to_address} with amount {amount} satoshis: {e}"),
            }
        },
        ...
        Commands::CreateWallet {
            ...
            Ok(mut new_wallet) => {
                println!("Created new wallet, starting sync");
        ...
        Commands::ImportPartialPrivateKeys {
            ...
            match wallet_manager.create_wallet_from_partial_keys(identifier, partial_private_keys.clone()) {
                Ok(mut new_wallet) => {
        ...
        Commands::ClearWallet {identifier} => {
            match wallet_manager.clear_wallet(identifier) {
                Ok(_) => println!("Cleared wallet"),
                Err(e) => eprintln!("Error clearing wallet: {e}"),
            }
        },
        Commands::ClearAllWallets => {
            match wallet_manager.clear_all_wallets() {
